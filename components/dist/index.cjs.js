...
function TokenBalances(props) {
  ...
  const columns = [{
    ...
    dataIndex: 'logo_url',
    key: 'logo_url',
    render: text => /*#__PURE__*/React__default["default"].createElement("img", {
      src: text,
    ...
    render: (_, item) => /*#__PURE__*/React__default["default"].createElement("td", null, Number.isInteger(item.balance / 10 ** item.contract_decimals) ? item.balance / 10 ** item.contract_decimals : (item.balance / 10 ** item.contract_decimals).toFixed(4))
  }, {
    title: 'Type',
    dataIndex: 'type',
    key: 'type',
